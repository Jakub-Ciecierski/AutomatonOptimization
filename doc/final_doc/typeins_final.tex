...
In this section the project implementation is discussed. 

The figure~\ref{fig:class_automata} presents the Automata module. The \textit{DFA} class represents a Deterministic Finite Automaton which includes a Transition Table. DFA contains a method which checks whether a input transition table is deterministic. The Algorithms module includes a DFA Reduction functionality. Firstly \textit{getReachabilityFlags} returns a vector informing whether a corresponding state is reachable or not. The \textit{reduceDFA} removes all states that are not flagged in the input vector. Finally \textit{removeUnreachableStates} encapsulates both previous functions to reduce the automaton by removing all unreachable states.

In figure~\ref{fig:class_opt_main} the Optimization module is shown. \textit{Optimizer} is responsible for starting \textit{PSO}, computing relation and errors. \textit{ParticleDecoder} is used to decode a particle into a transition function. Finally the \textit{ParticleFactory} is responsible for generating particles for \textit{PSO}.

Optimization algorithms are presented in figure~\ref{fig:class_opt_alg}. The classes: \textit{PSOFitness}, \textit{PSONeighbourhood} and \textit{PSOUpdate} are all responsible for calculating their respective roles in PSO computations.

A state diagram of finding the optimal automaton is shown in figure~\ref{fig:state_opt}. Moreover, the details of PSO process is presented in figure~\ref{fig:state_pso}.
...
\begin{figure}[H]
    ...
    \includegraphics[width=0.65\textwidth]{res/uml/classes/automata.jpg}
\end{figure}
...
\begin{figure}[H]
    ...
    \includegraphics[width=0.75\textwidth]{res/uml/classes/optimizationMain.jpg}
...
\begin{figure}[H]
    ...
    \includegraphics[width=0.75\textwidth]{res/uml/classes/optimizationAlg.jpg}
...
Training and testing sets were generated in two ways. Firstly, training set distinguished two subsets: all possible words with length up to constant $C=4$ and set of words longer than $4$ and not shorter than $20$. The entire training set had $2000$ words. The testing set contained $5000$ words with minimum length $21$ and maximum $30$. For convenience we will denote this set of words $C=4$.

Second, larger set of words contained training set having all words up to length $5$ and words longer than $5$ but not shorter than $20$. In total training set contained $8000$ words. Testing set contained $10000$ words with minimum length 21 and maximum 30. Let us denote this set by $C=5$.

In both cases the alphabet size was equal to $5$, maximum iteration of PSO was $100$ and the swarm size $50$.

%--------------------------------------------------------------------------------------
%--------------------------------------------------------------------------------------
\subsection{Reconstruction}
In reconstruction experiments four classes of automata, having $4,6,10,15$ states respectively, were considered. Ten automata in each class were generated randomly. Automata with unreachable states was discarded and generated again.

The PSO algorithm was ran for states in the continues interval: $[3,15]$.

%--------------------------------------------------------------------------------------
%--------------------------------------------------------------------------------------
\subsection{Approximation}
Similarly to reconstruction. four classes, each having ten automata, were generated. This time the number of states were much higher: $20, 30, 50, 80$ respectively.

The PSO algorithm was ran for states: ${4,6,8,10,12}$.

%--------------------------------------------------------------------------------------
\section{Results}
In this section results to our experiments are introduced. The results compare the accuracy of finding automaton of different classes and the accuracy depending on different set words: $C=4$ and $C=5$(c.f. section~\ref{sec:exp}). The accuracy was computed on two subsets of training set, on the union of both and on testing set.

%--------------------------------------------------------------------------------------
%--------------------------------------------------------------------------------------
\subsection{Reconstruction}
Results of all ten automata in each class have been averaged and presented in figure~\ref{fig:recons_results}. Each plot shows results for a single class and compares behavior for two kinds of input words, $C=4$ and $C=5$.

The accuracy within a single class is nearly the same across all sets of words. It is also clear to see that the accuracy grows with the growth of the number of states that the input automaton had. The class of automata with 15 states had the highest accuracy.

The figure~\ref{fig:recons_pso} shows the results in a form of testing set accuracy of each iteration of PSO from the interval [3,15]. The algorithm was not successful in finding automata with proper number of states for classes 4,6,10. The accuracy grew together with increasing the number of states in the output automata. Moreover, judging by the growth of the give function, it is probable that class of 15 automata would receive even higher accuracy for bigger number of states than 15.

Finally the time of computations of each class for both $C=4$ and $C=5$ is presented in figure~\ref{fig:recons_time}.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure}[h]
    \caption{Reconstruction. Testing Set Accuracy across the PSO interval for all classes}
    \label{fig:recons_pso}
    ...
    \nextgroupplot[title=Class 4, ylabel=Testing Set Accuracy]
    ...
    % CLASS     10
    %%%%%%%%%%%%%%%%%%%%
    \nextgroupplot[title=Class 10, ylabel=Testing Set Accuracy]
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure}[H]
    \caption{Reconstruction. Time of computations (in minutes) for classes 4,6,10,15 for both set of words}
    \label{fig:recons_time}
    ...
    \nextgroupplot[title=Time of Computations, ylabel=Time [min]]
...
%--------------------------------------------------------------------------------------
\subsection{Approximation}
As in the case of reconstruction the results are presented in the same form.
Figure~\ref{fig:approx_results} presents the results for all classes. Again, the results for all set of words seems to be stable.

The tendency of accuracy growth together with increased number of states remain the same. This is proved in the figure~\ref{fig:approx_pso} which shows us that the accuracy of the output automaton is higher for higher number of states.
...
\begin{figure}[H]
    ...
    \caption{Approximation Results. Automata Classes 20, 30, 50, 80}
...
\caption{Approximation. Testing accuracy across the PSO interval. Classes 20, 30, 50, 80}
\label{fig:approx_pso}
...
\nextgroupplot[title=Class 20,  ylabel=Testing Set Accuracy]
% APPROX_C4_CL20_1
\addplot[color=bblue, mark=x] coordinates {
	(4, 0.73)
	(6, 0.80)
...
% DFA ID    1
%%%%%%%%%%%%%%%%%%%%
\nextgroupplot[title=Class 50, ylabel=Testing Set Accuracy]
% APPROX_C4_CL50_1
\addplot[color=bblue, mark=x] coordinates {
	(4, 0.74)
	(6, 0.82)
